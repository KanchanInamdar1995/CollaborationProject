myModule.controller("FriendController",function($scope,$http,$rootScope,$location)
{
	...
	{
		$http.get('http://localhost:1567/SocialHubMiddleware/deleteFriendRequest/'+friend.friendId)
	...
	{
		$http.get('http://localhost:1567/SocialHubMiddleware/acceptFriendRequest/'+friend.friendId)
		.then(function(response)
		{
	...
	$scope.sendFriendRequest=function(friend)
	{
		...
		$http.post('http://localhost:1567/SocialHubMiddleware/sendFriendRequest/',$scope.friend)
		.then(function(response)
		{
	...
	{
		$http.get('http://localhost:1567/SocialHubMiddleware/showFriendList/'+$rootScope.currentUser.loginname)
		.then(function(response)
		{
	...
	{
		$http.get('http://localhost:1567/SocialHubMiddleware/showSuggestedFriendList/'+$rootScope.currentUser.loginname)
		.then(function(response)
		{
	...
	{
		$http.get('http://localhost:1567/SocialHubMiddleware/showPendingFriendRequest/'+$rootScope.currentUser.loginname)
		.then(function(response)
		{
